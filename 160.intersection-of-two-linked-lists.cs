 ...
 */
/*
 * Time: O(m + n) where m = length of List A, n = length of List B, Space: O(1)
 */
public partial class Solution
{
    public ListNode GetIntersectionNode(ListNode headA, ListNode headB)
    {
        ListNode a = headA;
        ListNode b = headB;
        // ListNode a go through List A combine List B
        // ListNode b go through List B combine List A
        // their intersection are the same position
        // if no intersection, then they will become null at the end
        while (a != b)
        {
