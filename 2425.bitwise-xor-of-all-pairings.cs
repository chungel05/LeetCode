...
public partial class Solution
{
    public int XorAllNums(int[] nums1, int[] nums2)
    {
        ...
        // so if nums2.Length is even, we will have even no. of nums1 elements, which after XOR is 0
        // so we only care about the nums2.Length is odd, then we XOR all elements in nums1
        if (nums1.Length % 2 == 1)
        {
