...
public partial class Solution
{
    public int[] ApplyOperations(int[] nums)
    {
        ...
        for (int i = 0; i < nums.Length; i++)
        {
            ...
            // if nums[i] is last element or nums[i] != nums[i + 1]
            // just mark nums[i] to nums[j] and move j pointer by 1
            ...
            // if nums[i] == nums[i + 1]
            // do the operation and mark nums[i] to nums[j]
            // then move j pointer by 1
            nums[j++] = nums[i] * 2;
            // move i pointer because next nums[i] is 0
            i++;
        }

        // mark the remaining nums[j] to 0 until the array is ended
        while (j < nums.Length)
            nums[j++] = 0;
