...
{
    private int NextCircularArrayLoop(int[] nums, int i)
    {
    ...
    public bool CircularArrayLoop(int[] nums)
    {
        ...
        {
            // first case: slow = i, fast = i.next
            int slow = i;
            int fast = NextCircularArrayLoop(nums, i);

            // check nums[slow] and nums[fast] are same direction and
            // check nums[slow] and nums[fast.next] are same direction
            while (nums[slow] * nums[fast] > 0 && nums[slow] * nums[NextCircularArrayLoop(nums, fast)] > 0)
            {
                if (slow == fast)
                {
                    if (slow != NextCircularArrayLoop(nums, slow))
                        return true;
                    // if it is one element loop, break
                    break;
                }

                slow = NextCircularArrayLoop(nums, slow);
                fast = NextCircularArrayLoop(nums, NextCircularArrayLoop(nums, fast));
            }
        }
        return false;
    }
