/*
 * @lc app=leetcode id=2013 lang=csharp
 *
 * [2013] Detect Squares
 *
...
public class DetectSquares
{
    ...
    public void Add(int[] point)
    {
        ...
        if (!count.ContainsKey((xAxis, yAxis)))
            count[(xAxis, yAxis)] = 0;
    ...
    public int Count(int[] point)
    {
        ...
        {
            if (Math.Abs(p[0] - xAxis) != Math.Abs(p[1] - yAxis) || p[0] == xAxis || p[1] == yAxis)
                continue;

            res += count.GetValueOrDefault((p[0], yAxis)) * count.GetValueOrDefault((xAxis, p[1]));
        }
        return res;
    }
}
