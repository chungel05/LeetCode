/*
 * @lc app=leetcode id=1438 lang=csharp
 *
 * [1438] Longest Continuous Subarray With Absolute Diff Less Than or Equal to Limit
 *
 * https://leetcode.com/problems/longest-continuous-subarray-with-absolute-diff-less-than-or-equal-to-limit/description/
 *
 * algorithms
 * Medium (56.63%)
 * Likes:    4134
 * Dislikes: 198
 * Total Accepted:    243K
 * Total Submissions: 429K
 * Testcase Example:  '[8,2,4,7]\n4'
 *
 * Given an array of integers nums and an integer limit, return the size of the
 * longest non-empty subarray such that the absolute difference between any two
 * elements of this subarray is less than or equal to limit.
 * 
 * 
 * Example 1:
 * 
 * 
 * Input: nums = [8,2,4,7], limit = 4
 * Output: 2 
 * Explanation: All subarrays are: 
 * [8] with maximum absolute diff |8-8| = 0 <= 4.
 * [8,2] with maximum absolute diff |8-2| = 6 > 4. 
 * [8,2,4] with maximum absolute diff |8-2| = 6 > 4.
 * [8,2,4,7] with maximum absolute diff |8-2| = 6 > 4.
 * [2] with maximum absolute diff |2-2| = 0 <= 4.
 * [2,4] with maximum absolute diff |2-4| = 2 <= 4.
 * [2,4,7] with maximum absolute diff |2-7| = 5 > 4.
 * [4] with maximum absolute diff |4-4| = 0 <= 4.
 * [4,7] with maximum absolute diff |4-7| = 3 <= 4.
 * [7] with maximum absolute diff |7-7| = 0 <= 4. 
 * Therefore, the size of the longest subarray is 2.
 * 
 * 
 * Example 2:
 * 
 * 
 * Input: nums = [10,1,2,4,7,2], limit = 5
 * Output: 4 
 * Explanation: The subarray [2,4,7,2] is the longest since the maximum
 * absolute diff is |2-7| = 5 <= 5.
 * 
 * 
 * Example 3:
 * 
 * 
 * Input: nums = [4,2,2,2,4,4,2,2], limit = 0
 * Output: 3
 * 
 * 
 * 
 * Constraints:
 * 
 * 
 * 1 <= nums.length <= 10^5
 * 1 <= nums[i] <= 10^9
 * 0 <= limit <= 10^9
 * 
 * 
 */
/*
 * Time complexity: O(n log n), space: O(n)
 */

// @lc code=start
public partial class Solution
{
    public int LongestSubarray(int[] nums, int limit)
    {
        PriorityQueue<int, int> maxheap = new PriorityQueue<int, int>();
        PriorityQueue<int, int> minheap = new PriorityQueue<int, int>();
        int l = 0, res = 0;

        for (int r = 0; r < nums.Length; r++)
        {
            // find out max num and min num in 2 heap
            maxheap.Enqueue(r, -nums[r]);
            minheap.Enqueue(r, nums[r]);

            // if (max - min > limit), we need to move left point
            // try to reduce max num / increase min num
            // in order to meet the limit
            while (nums[maxheap.Peek()] - nums[minheap.Peek()] > limit)
            {
                while (maxheap.Count > 0 && maxheap.Peek() <= l)
                    maxheap.Dequeue();
                while (minheap.Count > 0 && minheap.Peek() <= l)
                    minheap.Dequeue();
                l++;
            }

            // update res
            res = Math.Max(res, r - l + 1);
        }
        return res;
    }
}
// @lc code=end

