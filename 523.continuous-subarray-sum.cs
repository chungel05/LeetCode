...
/*
 * Concept:
 * each prefix Sum can be rewrite as n * k + remainder
 * if two prefix Sum are the same remainder:
 * prefixSum[i] - prefixSum[j] (which is sum between i and j)
 * = n2 * k + remainder - (n1 * k + remainder)
 * = k * (n2 - n1) which is multiple of k
 * then check i - j >= 2 or not
 * Time: O(n), Space: O(n)
 */
